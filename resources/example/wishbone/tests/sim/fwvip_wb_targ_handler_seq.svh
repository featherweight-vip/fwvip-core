...
class fwvip_wb_targ_handler_seq extends uvm_sequence #(fwvip_wb_seq_item);
  ...
  task body();
    ...
    begin
      fwvip_wb_seq_item final_item = fwvip_wb_seq_item::type_id::create("final");
      final_item.rsp_dat = item.we ? '0 :
                           (mem_model.exists(item.adr[31:0]) ?
                            {32'b0, mem_model[item.adr[31:0]]} : '0);
      final_item.rsp_err = 1'b0;
